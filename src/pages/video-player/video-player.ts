...
export class VideoPlayerPage implements AfterViewInit {
  ...
  constructor(
    ...
    if (!w.YT.loading) {
      ...
      (function() {
        ...
        a.src =
          "https://s.ytimg.com/yts/jsbin/www-widgetapi-vflQKB5wA/www-widgetapi.js";
        a.async = true;
    ...
    if (!w.onYouTubeIframeAPIReady) {
      w.onYouTubeIframeAPIReady = function() {
        w["player" + w.videoCuentoId] = new w.YT.Player(
          "player" + w.videoCuentoId,
          {
            events: {
  ...
  ngAfterViewInit() {
    ...
    if (this.w.youTubeIframeAPIReady === true) {
      this.w["player" + this.w.videoCuentoId] = new this.w.YT.Player(
        "player" + this.w.videoCuentoId,
        {
          events: {
